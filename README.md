...
multiple Unix-like operating systems and quickly set up environments on new hosts. Colloquially this is referred to as dotfile management.

This README covers how I manage these dotfiles and how they're used to configure my
environment:
...
are worth checking out. The only *wrong* choice is to not use a tool at all, and while
I'm biased toward the strategy described below, I'd recommend any strategy that stands up
to the cost-benefit analysis of [xkcd #1205].
...
cloning this repository. Familiarity with package managers and Git are assumed, but both
have excellent documentation for anyone just getting started.

I chose these tools based on the following requirements:

1. Dependencies should be easy to bootstrap on popular Unix-like operating systems.
   They should be available through a package manager whenever possible.
2. The learning curve for each dependency should be shallow.
3. Configuration for dependencies should be optional, not required.

As a GNU tool, Stow is consistently available with package managers and it can also be
[installed from source][stow-installation] (it's just a Perl module). Stow also follows
the Unix philosophy of doing one thing and doing it well, which makes it easy to learn.
Lastly, my Stow configuration is limited to [ignore lists][stow-ignore-lists] and
[resource files][stow-resource-files] to define default command-line options. Both of
these are optional.
...
Stow was originally created to solve a problem unrelated to dotfile management:
...
actually needed to be removed. If you were lucky, you might've had access to a `make`
target to do this for you. Stow, however, can remove its symbolic links and effectively
uninstall Perl with a single command.
...
structure below our home directory:
...
The names of the stow and package directories are flexible, but typically the stow
directory will match the name of your Git repository and your package directories will be
organized by tool.

Next we can use a single Stow command to expose this file to Git. The `git` argument for
this command refers to the package directory and the `-v` option lets us to see what
Stow is doing:
...
Stow can also create symlinks in target directories specified with the `-t` option,
including grandparent or sibling directories. This gives us the flexibility to locate our
dotfiles directory somewhere else in the file system, including directories within other
stow directories (more on this below).
...
`.stowrc` should be modified to refer to your target directory if you're forking this
repository.
...
By using nested stow directories, I'm able to layer configuration based on my current
...
it's a game-changer for anyone who develops often in a terminal.
...
* powerlevel10k targets the Zsh prompt only. I prefer a minimal prompt theme, so the
  overhead of another large dependency isn't worth it to me.
